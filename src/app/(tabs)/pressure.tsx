...
export default function PressureTabScreen() {
  ...
  const { pressureReadings, refresh: refreshRecords, isLoading: recordsLoading, deletePressure } = useRecordManagement();
  ...
  function confirmDeleteReading(readingId: number) {
    Alert.alert(
      'Excluir medição',
      'Deseja excluir esta leitura de pressão? Essa alteração também será sincronizada com a nuvem quando disponível.',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: () => {
            void (async () => {
              await deletePressure(readingId);
